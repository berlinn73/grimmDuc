...
import com.grimm.repository.JpaRepository;
...
import com.grimm.util.pool.ConnectionPool;
...
public class BasicQuery<T, ID extends Serializable> implements JpaRepository<T, ID> {
    ...
    public <S extends T> List<S> findByCondition(String condition) {
        Connection connection = ((ConnectionPoolImpl) (connectionPool)).takeOut();
        StringBuilder sql = new StringBuilder(Query.SELECT).append(AnnotationUtil.getTableName(tClass)).append(Query.WHERE).append("id_user = " + condition + "");
        PreparedStatement ps = null;
        try {
            ps = connection.prepareStatement(sql.toString());
            ResultSet resultSet = ps.executeQuery();
